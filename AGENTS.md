...
1. 每次修改代码时，必须同步更新项目根目录下的 `CHANGELOG.md`，未记录不得视为任务完成。
2. 每次新增变更记录时，使用标题 `## YYYY-MM-DD version:<版本号>`，例如：`## 2026-09-01 version:0.1.2`。优先使用项目现有版本号；若不存在版本号，则从 `0.0.1` 开始创建。
3. 所有变更说明必须使用中文，格式为：`- <短提交哈希>: <中文变更说明>`。
4. 如果代码尚未提交，暂时使用 `pending`；提交后替换为真实的短提交哈希，禁止编造提交哈希。
