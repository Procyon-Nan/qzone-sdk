...
- `logger` 只接收阶段、端点名、耗时、重试次数、HTTP 状态和错误码等白名单
  字段，不包含请求正文、响应正文或凭据。`request.error` 表示请求失败未被读取
  回退机制消费，或最终备用端点失败；`read.fallback` 表示 SDK 正在切换到兼容
  读取端点，不代表登录态失效，也不要求调用方重试。`fallbackEndpoint` 仅为 SDK
  内部端点标识，不包含 URL 或账号信息。
